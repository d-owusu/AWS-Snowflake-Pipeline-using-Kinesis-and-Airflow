...
task_customers_landing_to_processing = BashOperator(
 ...
 bash_command='aws s3 mv s3://snflakedata/firehose/customers/landing2022/ s3://snflakedata/firehose/customers/processing/{0}/ --recursive'.format(batch_id),
 dag=dag
)
 
task_customers_processing_to_processed = BashOperator(
 task_id="customer_processing_to_processed",
 bash_command='aws s3 mv s3://snflakedata/firehose/customers/processing/{0}/ s3://snflakedata/firehose/customers/processed/{0}/ --recursive'.format(batch_id),
 dag=dag
)
...
task_orders_landing_to_processing = BashOperator(
    ...
    bash_command = 'aws s3 mv s3://snflakedata/firehose/orders/landing2022/ s3://snflakedata/firehose/orders/processing/{0}/ --recursive'.format(batch_id),
...
[bash_task >>  task_customers_landing_to_processing >> snowflake_customers_sql_str >> task_customers_processing_to_processed ,
 bash_task >>task_orders_landing_to_processing >> snowflake_orders_sql_str >> task_orders_processing_to_processed]  >> post_task
